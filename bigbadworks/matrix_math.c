...
int matrix_mul(matrix *m1, const matrix *m2) {
    ...
    // Создаем временную матрицу для результата
...
int matrix_mul2(matrix *m, const matrix *m1, const matrix *m2) {
    ...
    // Проверяем, не совпадает ли m с m1 или m2
    if (m == m1 || m == m2) {
        // Используем временную матрицу
...
void matrix_transpose(matrix *m) {
    ...
    // Для квадратной матрицы - на месте
    if (matrix_rows(m) == matrix_cols(m)) {
        ...
        // Для прямоугольной создаем новую и копируем
...
int matrix_gauss_solve(const matrix *A, const matrix *B, matrix *X) {
    if (!A || !B || !X) return -1;
    if (matrix_rows(A) != matrix_cols(A)) return -1;  // A должна быть квадратной
    if (matrix_rows(A) != matrix_rows(B)) return -1;  // Одинаковое количество уравнений
...
matrix *matrix_exp(const matrix *A, double eps) {
    ...
    // E + A
    matrix *result = matrix_alloc_id(n);  // E
    if (!result) return NULL;
    
    matrix *term = matrix_copy(A);  // A
    if (!term) {
        matrix_free(result);
        return NULL;
    }
    
    matrix_add(result, term);  // result = E + A
    
    double norm_term = matrix_norm(term);
    size_t k = 2;  // начинаем с A^2/2!
